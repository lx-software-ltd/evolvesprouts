...
export interface paths {
    ...
    "/v1/admin/audit-logs": {
         ...
         * @description Returns database audit history (trigger- and application-sourced rows in `audit_log`). Optional filters combine as in the admin Lambda handler: when `record_id` is set, `table` is required; `user_id` is a Cognito subject (sub), `api-key:<id>`, or a system actor (`system`, `webhook:whatsapp`, `webhook:meta`). Use `email` to filter by Cognito email, API key name, or system-actor label (server resolves to sub, `api-key:<id>`, or the system `user_id`); do not pass `user_id` together with `email`.
         */
        get: {
            parameters: {
                query?: {
                    ...
                    record_id?: string;
                    user_id?: string;
                    /** @description Filter by Cognito email, API key name, or system-actor label. Values containing `@` resolve to a Cognito `sub` via `list_users`. Other values first match known system actors (`System`, `WhatsApp webhook`, `Meta webhook`, or stored ids `system`, `webhook:whatsapp`, `webhook:meta`), then `api_keys.name` (case-insensitive) which becomes `api-key:<id>`. Mutually exclusive with `user_id`. When no actor matches, the response is an empty list. */
                    email?: string;
...
export interface components {
    schemas: {
        ...
        AuditLog: {
            ...
            /** @enum {string} */
            action: "INSERT" | "UPDATE" | "DELETE";
            /** @description Cognito subject, `api-key:<id>` for token writes, or a system actor (`system`, `webhook:whatsapp`, `webhook:meta`). */
            user_id?: string | null;
