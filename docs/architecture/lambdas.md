...
- Trigger: API Gateway — currently wired for
  ...
  previous object fails after commit). Creating or updating an asset to
  `visibility=public` also creates a share-link row (when none exists) using
  `ASSET_SHARE_LINK_DEFAULT_ALLOWED_DOMAINS`; missing defaults are logged and
  the asset write still succeeds. Expense- and customer-invoice-tagged assets
  ...
  `POST /v1/admin/billing/invoices/{id}/issue` creates a pending inbound `customer_payments` row for positive-total invoices (amount and currency from the invoice, method `fps`; `enrollment_id` only when the invoice has exactly one enrollment line; customized or multi-enrollment invoices leave it unset; no allocation); zero-total issues skip that payment stub. Enrollment lines on issued invoices: **zero-total** issues immediately transition linked enrollments from `registered` to `confirmed` and promote the enrolled CRM party when it is still `prospect` to `client` (family or organization enrollments: every contact member of that family or organization); **positive-total** issues leave enrollment status unchanged until the first **payment allocation** is created against that issued invoice, which performs the same `registered`→`confirmed` and prospect→client promotions idempotently;
...
- Purpose:   asset metadata CRUD (admin asset list returns `linked_tag_names` for tag
  ...
  sales pipeline lead management (list/detail/create/update/notes/export/analytics/daily-plan),
...
- Handler: backend/lambda/sales_daily_plan/handler.py
- Stack: nested stack `evolvesprouts-Messaging`
...
- Environment:
  ...
  - `OPENROUTER_API_KEY_SECRET_ARN`, `OPENROUTER_CHAT_COMPLETIONS_URL`,
    `OPENROUTER_MODEL` (fallback for sales AI when `sales_settings.openrouter_model` is unset)
