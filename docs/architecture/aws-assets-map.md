...
| `/v1/admin/cognito-users` | GET, POST | Admin Group | `EvolvesproutsAdminFunction` | Paginated Cognito user-pool listing (`name`, `email`, `cursor`, `limit`); create user in optional staff group |
| `/v1/admin/cognito-users/{username}` | GET, PATCH, DELETE | Admin Group | `EvolvesproutsAdminFunction` | Get, update (email/name/group/enabled), or delete a Cognito user |
...
| `/v1/admin/assets/{id}/grants/{grantId}` | DELETE | Admin Group | `EvolvesproutsAdminFunction` | Path part stays `{grantId}` so CloudFormation does not create a second variable sibling under `/grants` |
