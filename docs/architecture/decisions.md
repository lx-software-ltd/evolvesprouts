...
one non-manual open lead per `contact_id`. Migration `0085` adds
`action_recorded` on `lead_event_type` in its own transaction (PostgreSQL
cannot use a new enum label in the same txn). Migration `0086` merges
