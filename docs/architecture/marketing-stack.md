...
| Instagram DMs → CRM | Meta webhook on Admin Lambda (`/v1/meta/webhook`, `object: instagram`); Sales → Instagram inbox; Graph import of last 20 bodies/thread | Subscribe Instagram `messages`; Page token + `CDK_PARAM_META_PAGE_ID` for history import |
